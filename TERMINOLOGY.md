...
## B8-lifestyles-p4. Thuật ngữ đợt nhỏ 4 việc #8 `lifestyles/` — 4 file 345-439 dòng (numina_lifestyle, numina_sorcery, possessed, mokole)

Nguồn: 4 file dịch qua agent song song. **4 lỗi thật phát hiện khi merge** — đáng chú ý đợt này có 2 lỗi dạng mới (tự thêm bracket concept-link sai, và lệch tên với tiền lệ B6 dù B6 nằm ở thư mục khác `decisions/` không phải `lifestyles/` — xác nhận cần tra cứu xuyên toàn bộ repo, không chỉ trong cùng thư mục đang dịch):
1. `possessed`: dòng `deception_perk_effect_3` — bản gốc dùng chữ thường trần "schemes" (không bracket), agent tự thêm `[scheme|E]` — bracket thừa. Sửa lại dịch nghĩa "mưu đồ" không dùng concept-link.
2. `mokole`: "Dissolver" (tham số 1 của `Glossary('Dissolver','glossary_wyrm_tt')`, 5 lần) dịch thành "Kẻ Hủy Diệt" — SAI, tiền lệ đã chốt là "Kẻ Hòa Tan" (dòng ~1651, cùng nhóm với "Centipede"→"Rết"). Sửa lại cả 5 chỗ.
3. `mokole`: `Select_CString('point','points')` dịch thành "điểm đặc điểm" (lặp từ "đặc điểm" 2 lần trong 1 cụm, không tự nhiên) — sửa thành "điểm thiên phú" khớp mẫu đã dùng ở `investiture` (`MNESIS_PERK_COST_EFFECT`/`INVESTITURE_COST_EFFECT` cùng cấu trúc "This perk costs $VALUE$ perk point(s)").
4. `numina_sorcery`: file này liệt kê nhiều "Path" Sorcery hơn 14 trường phái đã chốt ở B6-decisions-sorcery (`decisions/POD_numina_sorcery_decisions_l_english.yml`), nhưng 6 tên TRÙNG key với B6 lại bị agent đặt tên khác — không tra cứu B6 vì B6 nằm ở thư mục `decisions/`, không nghĩ tới liên kết xuyên thư mục. Sửa lại 5 tên theo đúng B6: "Conjuration" Triệu Hồi Thuật→**Triệu Hoán Thuật**, "Enchantment" Ma Thuật Hóa→**Phù Chú Thuật**, "Ephemera" Linh Giới Thuật→**Linh Thể Thuật**, "Healing" Chữa Lành Thuật→**Trị Liệu Thuật**, "Necromancy" Tử Linh Thuật→**Chú Thuật Tử Linh** (khớp luôn với `POD_necro_magic_lifestyle` vừa dịch ở đợt 3, cùng khái niệm "Necromancy" xuất hiện ở 2 nơi). "Hellfire"→"Hỏa Ngục Thuật" agent đã dịch đúng khớp B6 sẵn, không cần sửa.

**Bài học mới rút ra từ lỗi #4:** khi 1 file thuộc thư mục A (`lifestyles/`) chứa các "Path"/trường phái/tên nhóm đã từng được liệt kê đầy đủ ở 1 file khác thuộc thư mục B (`decisions/`) đã dịch trước đó, PHẢI chủ động grep TERMINOLOGY.md tìm tên các khái niệm liên quan (không chỉ tin agent tự tra "chưa có tiền lệ") — coordinator tự phát hiện nhờ nhớ lại đã có mục B6-decisions-sorcery khi thấy tên các Path lạ, không phải do agent báo cáo.

| English | Tiếng Việt chốt | Ghi chú |
|---|---|---|
| Fortune, Illusion, Quintessence Manipulation, Shadows *(4 Path Sorcery KHÔNG nằm trong 14 trường phái đã chốt B6 — bổ sung riêng của `lifestyles/POD_numina_sorcery`)* | Vận Số Thuật, Ảo Ảnh Thuật, Thao Túng Tinh Chất, Bóng Tối Thuật | đặt mới, không trùng/mâu thuẫn B6 vì là 4 khái niệm khác biệt (không có trong danh sách 14) |
| Animal Psi, Anti Psi/Mind Shields, Astral Projection, Clairvoyance, Cyberkinesis, Cyberpathy, Ectoplasmic Generation/ectoplasm, Empathic Healing, Psychic Invisibility, Psychokinesis, Pyrokinesis, Synergy *(nhóm năng lực Psi bổ sung, `numina_lifestyle`)* | Ngoại Cảm Thú, Kháng Ngoại Cảm/Khiên Tâm Trí, Xuất Hồn, Thiên Lý Nhãn, Điều Khiển Điện Tử, Đọc Dữ Liệu Điện Tử, Sinh Chất Linh Khí/linh khí, Trị Liệu Cảm Thông, Tàng Hình Ngoại Cảm, Ý Động Lực, Khống Hỏa, Cộng Hưởng | đặt mới, ghép tiền tố "Ngoại Cảm" đã chốt B5-numina với các danh từ mô tả năng lực cụ thể |
| Fomori *(danh xưng phàm nhân bị Bane chiếm hữu)* | Fomori *(giữ nguyên)* | tái xác nhận, danh từ riêng WoD |
| Rite of Making, Berserker, Homogeneity, Dispersion, Malleate, Entropic Touch, Mask Presence, Skinshift, Blending *(nhóm sức mạnh Possessed/Fomori)* | Nghi Thức Tạo Tác (tái xác nhận), Cuồng Chiến, Đồng Hóa (tái xác nhận), Phân Tán, Nhũn Hóa, Chạm Suy Entropy, Ẩn Giấu Hiện Diện, Biến Đổi Da, Ngụy Trang | đặt mới; "Entropy" giữ nguyên (thuật ngữ vật lý, không có từ Việt súc tích tương đương) |
| Changing Breeds *(danh xưng chung mọi loài biến hình WoD — Garou/Bastet/Mokolé...)* | Changing Breeds *(giữ nguyên)* | đặt mới, danh từ riêng WoD |
| Mokolé-mbembe, Gumagan, Makara, Zhong Lung *(4 Stream/dòng huyết thống Mokolé, tái xác nhận)*, Homid/Suchid/Archid, Bandaiyan | giữ nguyên tiếng Anh | tái xác nhận |
| Rising/Noonday/Setting/Midnight/Shrouded/Decorated/Eclipsed Sun *(7 Auspice/giai đoạn mặt trời Mokolé)* | giữ nguyên tiếng Anh | đặt mới, danh xưng riêng theo lore, tương tự cách giữ nguyên Auspice Garou (Ragabash...) |
| Luna, Helios, Tellurian, Olodumare, Hsi Wang Mu, Wani, Bunyip, Matre, Gorgon, True Dragon *(tên riêng linh hồn/thực thể Mokolé)* | giữ nguyên tiếng Anh | đặt mới |
...
## B8-lifestyles-p9. Thuật ngữ `POD_wraith_lifestyle_l_english.yml` (1064/1064 dòng — file cuối cùng `lifestyles/`, hoàn tất việc #8 27/27)

Nguồn: dịch qua 6 agent chia đoạn theo ranh giới dòng trống (1-180, 181-366, 367-531, 532-716, 717-900, 901-1064). Toàn bộ 16 Arcanos "cổ điển" (Argos/Castigate/Embody/Fatalism/Flux/Inhabit/Intimation/Keening/Lifeweb/Mnemosynis/Moliate/Outrage/Pandemonium/Phantasm/Puppetry/Usury) + Thorn + nhóm "Jade Arcanoi" bổ sung (Chains of the Emperor, 5 "Way of the X", Blighted Insight/Contaminate/Corruption/False Life/Larceny/Maleficence/Shroud-Rending/Tempest-Weaving/Tempestos) + nhóm Guild Arcanoi mới (Fascinate/Serendipity/Behest/Connaissance/Displace) — tất cả đều là TÊN ARCANOS/tên nhánh, giữ nguyên tiếng Anh theo tiền lệ B4h/B5-wraith. Không có `UmbraGlossaryLocalized()` hay `Select_CString()` trong file này (chỉ `Glossary()` 2 tham số dùng lại key đã chốt "Dark Kingdom"/"Skinlands", và `UmbraGlossary()` 1 tham số không dịch).

**2 lỗi chéo giữa các đoạn tự phát hiện và sửa khi merge (không phải agent tự báo cáo hết, coordinator tự đối chiếu):**
1. Agent đoạn 1 (dòng 1-180) dịch `_focus_effect_desc` thành "**Chú tâm vào** X mang lại:" — lệch với mẫu đã dùng nhất quán ở TẤT CẢ file `lifestyles/` khác đã dịch trước đó (`lore_focus_effect_desc`, `fomori_focus_effect_desc`... đều dùng "**Tập trung vào** X mang lại:"), và cũng lệch với chính agent đoạn 2 (dòng 181-366) dùng đúng "Tập trung vào". Sửa lại 35 chỗ ở đoạn 1 khi merge.
2. Agent đoạn 2 (dòng 181-366) để nguyên 8 dòng `arcanos_X_focus: "X Focus"` — **hoàn toàn chưa dịch** (chỉ dịch phần `_focus_desc`/`_focus_effect_desc` đi kèm, bỏ sót chính tên `_focus_name`) — trong khi agent đoạn 1 dịch đúng mẫu `_focus_name` thành "Trọng Tâm X" cho 29 Arcanos liền trước. Sửa lại 8 chỗ (`arcanos_shroud_rending_focus`, `arcanos_tempest_weaving_focus`, `arcanos_tempestos_focus`, `arcanos_fascinate_focus`, `arcanos_serendipity_focus`, `arcanos_behest_focus`, `arcanos_connaissance_focus`, `arcanos_displace_focus`) thành "Trọng Tâm X" khi merge.
3. `Hive-Mind` (tên Arcanos, `arcanos_hive_mind_focus`) — agent đoạn 1 giữ nguyên tiếng Anh theo mẫu tên Arcanos khác (Contaminate/Corruption/False Life...), nhưng agent đoạn 5 (dòng 717-900, nơi định nghĩa chính `hive_mind_visible`) tra đúng tiền lệ B4v (`Hive Mind` = "Tâm Trí Bầy Đàn", đã chốt cho khái niệm house_of_worship của Spectre ở `religion/POD_religion_wyrm`) và dịch nghĩa. Khác với Contaminate/Corruption/False Life (chưa từng có tiền lệ dịch nghĩa nên giữ nguyên là đúng), "Hive Mind" ĐÃ có tiền lệ dịch nghĩa từ trước — nên **ưu tiên áp dụng tiền lệ đã chốt thay vì giữ nguyên theo mẫu Arcanos khác**. Đồng bộ lại `arcanos_hive_mind_focus`/`_focus_effect_desc` ở đoạn 1 từ "Hive-Mind" sang "Tâm Trí Bầy Đàn" khi merge.

**Bài học bổ sung cho các đợt merge nhiều-agent sau này:** khi 2 đoạn liền kề cùng dịch một PATTERN LẶP LẠI xuyên suốt (như 29+8=37 dòng `arcanos_X_focus`/`_focus_effect_desc` ở đây), phải tự trích xuất TOÀN BỘ các dòng khớp pattern đó từ mọi đoạn ra so sánh song song (không chỉ đọc lướt từng đoạn riêng lẻ) — lỗi #1 và #2 ở trên chỉ lộ ra khi coordinator tự grep gom nhóm `_focus:`/`_focus_effect_desc:` xuyên suốt cả 6 file scratch cùng lúc, không phải khi đọc từng đoạn riêng.

| English | Tiếng Việt chốt | Ghi chú |
|---|---|---|
| `_focus_name` *(mẫu `arcanos_X_focus: "X Focus"`, lặp 37 lần toàn file)* | Trọng Tâm X | ✅ tái xác nhận mẫu đã dùng xuyên suốt `lifestyles/` (`lore_focus`, `fomori_focus`...); tên Arcanos X bên trong GIỮ NGUYÊN tiếng Anh |
| `_focus_effect_desc` *(mẫu `"Focusing on X grants:"`)* | Tập trung vào X mang lại: | ✅ tái xác nhận mẫu đã dùng xuyên suốt `lifestyles/` — KHÔNG dùng "Chú tâm vào" |
| Hive-Mind/Hive Mind *(tên Arcanos, `arcanos_hive_mind_focus`/`hive_mind_visible`)* | Tâm Trí Bầy Đàn | ✅ tái xác nhận B4v — ưu tiên tiền lệ dịch nghĩa đã chốt, khác các Arcanos Jade khác (Contaminate/Corruption/False Life/Blighted Insight/Larceny/Maleficence/Shroud-Rending/Tempest-Weaving/Tempestos) chưa có tiền lệ nên giữ nguyên tiếng Anh |
| Chains of the Emperor, Way of the Scholar/Farmer/Artisan/Merchant/Soul *(5+1 tên nhánh Arcanoi mở rộng, lần đầu xuất hiện)* | giữ nguyên tiếng Anh | đặt mới; nhất quán mẫu "tên Arcanos = danh từ riêng, không dịch" của B4h |
| Emperor/Scholar/Farmer/Artisan/Merchant/Soul *(6 caste tương ứng 6 "Way of the X", `_visible` key)* | Hoàng Đế/Học Giả/Nông Dân/Nghệ Nhân/Thương Nhân/Linh Hồn | đặt mới, dịch nghĩa tên caste (khác tên nhánh Arcanoi giữ nguyên ở trên) |
| Harbinger, Pardoner, Proctor, Oracle, Artificer, Chanteur, Monitor, Masquer *(8 Guild Arcanoi xuất hiện trong file này, tên perk "A [Guild] can...")* | Sứ Giả, Người Xá Tội, Giám Thị, Nhà Tiên Tri, Thợ Chế Tác, Ca Sĩ, Người Giám Sát, Thợ Nhào Nặn | ✅ tái xác nhận tiền lệ đã chốt (15 Guild Arcanoi, xem mục Guild trong TERMINOLOGY.md phần Wraith) |
| Tvashtriya *(Guild của Displace, "A Tvashtriya can slip into the Corpus of a wraith...")* | Tvashtriya *(giữ nguyên)* | đặt mới; danh từ riêng WoD (tên tiếng Phạn), không có nghĩa dịch tự nhiên rõ ràng |
| Escher's Stair, Morphean Embrace *(2 tên power tái dùng từ `interactions/`)* | Cầu Thang Escher, Vòng Tay Morphean | ✅ tái xác nhận B5-wraith-p6 |
| Driving Urge *(tên perk Fascinate, `driving_urge_perk_name`)* | Thôi Thúc Nội Tâm | ✅ áp dụng theo B5-wraith-p5 (ưu tiên vì đây là bản dịch gần với tên key gốc hơn B5-wraith-p4 "Thôi Thúc Mãnh Liệt" cho cùng power — 2 tiền lệ khác nhau tồn tại ở `interactions/`, chưa thống nhất, ghi nhận lại để rà soát sau) |
...
## B12-single-combat. Thuật ngữ `single_combat_POD_l_english.yml` (1988/1988 dòng) — việc #12 (tiếp)

Nguồn: file mô tả flavor-text cận chiến 1v1 (combat move option/tooltip/feedback/injury/fatality theo Discipline/Fera/Chi/Egyptian magic). Dịch qua 7 agent chia đoạn (1-245, 246-566, 567-879, 880-1090, 1091-1412, 1413-1726, 1727-1988), coordinator merge bằng Python (đối chiếu key-tuần-tự, dòng trống byte-for-byte, token count, `UmbraGlossaryLocalized()` tham số với `git show HEAD`). Toàn bộ 7 đoạn khớp key-tuần-tự/dòng trống ngay từ lần verify đầu — không lỗi cấu trúc dòng (nhờ prompt đã có yêu cầu so key-tuần-tự từ đợt effects_POD/POD_journeys).

**⚠️ 15 lỗi thuật ngữ phát hiện và sửa khi merge — TOÀN BỘ đều là dịch nghĩa nhầm tên riêng Discipline lẽ ra phải GIỮ NGUYÊN tiếng Anh theo B0/B2481/B2558/B2565/B3008 (Potence/Celerity/Fortitude/Dominate/Oblivion... không dịch nghĩa dù xuất hiện giữa câu văn xuôi thường, khác với "Discipline" nói chung = "Dị Năng"):**
1. Đoạn 5 (1091-1412): "Celerity"→"Tốc Độ" sai, 6 chỗ (dòng 1208,1209,1210,1314,1315,1316). Sửa lại "Celerity".
2. Đoạn 5: "Potence"→"Sức Mạnh" sai, 4 chỗ (dòng 1211,1214,1317,1320). Sửa lại "Potence". **Lưu ý phân biệt:** dòng 1938-1939 `POD_combat_move_prowess_from_pain_wounded_*_modifier: "Prowess from Pain N"`→"Sức Mạnh Từ Cơn Đau N" là ĐÚNG (tên perk cụ thể đã chốt B2568, không phải tên Discipline Potence).
3. Đoạn 5: "Dominate"→"Sự Chi Phối" sai, 1 chỗ (dòng 1331). Sửa lại "Dominate".
4. Đoạn 4 (880-1090) + đoạn 6 (1413-1726): "Oblivion"→"Hư Vô" sai ở 3/11 chỗ xuất hiện trong key `POD_touch_of_oblivion` (dòng 913, 1234, 1340) — 8 chỗ còn lại của cùng key (97-99, 278, 383, 493, 599) đã dịch đúng giữ nguyên "Oblivion". **Phân biệt quan trọng:** "Oblivion" trong file này là tên Discipline (dòng 98 xác nhận rõ: `"You use your Oblivion powers to injure..."` — năng lực nhân vật SỬ DỤNG), khác hẳn "Oblivion" = "Hư Vô" đã chốt B3q/B4n/B4u/B4w cho khái niệm **negative_afterlife** (cõi hư vô/diệt vong trong lore Wraith) — hai nghĩa khác nhau của cùng 1 từ tiếng Anh, phải đọc ngữ cảnh câu để phân biệt, không áp dụng máy móc theo tần suất gặp nhiều hơn.
5. Đoạn 6: "Fortitude"→"Sự Bền Bỉ" sai, 1 chỗ (dòng 1104, key `POD_toughness.fatality`). Sửa lại "Fortitude".

**Bài học quan trọng nhất đợt này:** dù TERMINOLOGY.md đã chốt rất rõ "tên Discipline giữ nguyên tiếng Anh" (B0/B2481, nhắc lại ≥8 lần ở các mục sau), 2/7 agent (đoạn 5, đoạn 4+6) vẫn tự ý dịch nghĩa khi tên Discipline xuất hiện lồng trong câu văn xuôi mô tả hành động (thay vì ở vị trí "tên riêng đứng một mình" dễ nhận ra) — ví dụ "with the power of Celerity" bị đọc như tính từ mô tả thường ("sức mạnh của tốc độ") thay vì nhận ra "Celerity" là danh từ riêng. Cảnh báo cho lần sau: khi prompt liệt kê "các tên Discipline giữ nguyên", nên nhấn mạnh thêm rằng quy tắc này áp dụng NGAY CẢ KHI tên Discipline nằm giữa câu mô tả thông thường, không chỉ khi đứng riêng.

**Kết quả kiểm tra cuối:** sau khi sửa cả 15 chỗ, đối chiếu token với `git show HEAD` khớp tuyệt đối (`$ref$` 0, bracket 272, icon 1, mở tag 183, đóng tag 141, `\n` 28, `\"` 4, dòng 1988, BOM `efbbbf`). Key-sequence và dòng trống khớp byte-for-byte toàn bộ 1988 dòng. `UmbraGlossaryLocalized('shadowlands','Underworld')` (1 lần duy nhất trong file, dòng 196) dịch đúng thành tham số 2 = `'Âm Phủ'` theo B4j. Quét English-sót bằng heuristic (dòng có ≥4 từ Latin liên tục không dấu tiếng Việt sau khi loại bracket/ref/tag): 0 dòng nghi vấn.

## B12-regiments. Thuật ngữ `POD_regiments_l_english.yml` (729/729 dòng) — việc #12 (tiếp)

Nguồn: tên đơn vị quân sự MAA (men-at-arms) đặc thù POD + mô tả flavor (`_flavor` suffix). Dịch qua 4 agent chia đoạn (1-165, 166-383, 384-544, 545-729), coordinator merge bằng Python. Toàn bộ 4 đoạn khớp key-tuần-tự/dòng trống/token ngay từ lần verify đầu.

**1 lỗi merge phát hiện và sửa:** đoạn 3 (384-544) tự báo đã dịch đúng `UmbraGlossaryLocalized('yomi_wan','Yomi Hell')`→"Địa Ngục Yomi" và `('shadowlands','Underworld')`→"Âm Phủ" nhưng thực tế KHÔNG — cả 5 lệnh trong file (4× `yomi_wan`, 1× `shadowlands`, dòng 387/389/403/411) vẫn còn nguyên tiếng Anh ở tham số 2 khi coordinator đối chiếu HEAD. Sửa lại đúng theo B3c (`Yomi Hell`→"Địa Ngục Yomi") và B4j (`Underworld`→"Âm Phủ"). **Bài học:** báo cáo tự kiểm tra của agent (dù chi tiết, có vẻ đã verify) vẫn có thể sai — coordinator PHẢI tự chạy lại script đối chiếu `UmbraGlossaryLocalized()` với `git show HEAD` sau merge, không tin hoàn toàn báo cáo agent.

**Quy ước tên đơn vị (regiment name, không có hậu tố `_flavor`):** giữ nguyên tiếng Anh, nhất quán với `POD_regiments_spirits`/`POD_regiments_fera` đã dịch trước — chỉ dịch phần mô tả flavor text. 0 lỗi Discipline-name-in-prose ở file này (khác `single_combat_POD` ngay trước) — cả 4 agent đều tự rà soát đúng theo cảnh báo mới thêm vào prompt.

**Kết quả kiểm tra cuối:** đối chiếu token với `git show HEAD` khớp tuyệt đối (`$ref$` 14, bracket 96, icon 0, mở tag 309, đóng tag 308, `\n` 2, `\"` 0, dòng 729, BOM `efbbbf`). Key-sequence và dòng trống khớp byte-for-byte toàn bộ 729 dòng. 15 `Glossary()` (multiset tham số 2 khớp HEAD), 5 `UmbraGlossaryLocalized()` (multiset tham số 1 khớp HEAD, tham số 2 đã sửa đúng cả 5). Quét English-sót: 9 dòng nghi vấn, cả 9 đều false positive (tên regiment cố ý giữ nguyên tiếng Anh).

## B12-struggle. Thuật ngữ `struggle_POD_l_english.yml` (490/490 dòng) — việc #12 (tiếp)

Nguồn: cơ chế "Struggle" (Xung Đột, B5 dòng 196) giữa các phe phái nội bộ Banu Haqim (Web of Knives/Ashirra/Council of Scrolls/Silsila/Lost Tribe) và Struggle Pháp/Anh thời Trung Cổ (French Struggle). Dịch qua 3 agent chia đoạn (1-153, 154-325, 326-490), coordinator merge bằng Python. Toàn bộ 3 đoạn khớp key-tuần-tự/dòng trống/token ngay từ lần verify đầu.

**⚠️ Phát hiện xung đột thuật ngữ nghiêm trọng — "the Embrace" có tới 4 bản dịch khác nhau tồn tại song song trong repo:** "Nghi lễ Trao truyền"/"Trao truyền" (glossary_POD, nguồn gốc `pod_Embrace`/`pod_Embraced`, dòng 263 — lan truyền rộng nhất, 15 file đã commit), "Hiến Máu" (`game_POD_concepts.game_concept_Embrace`/`_desc`, dòng 3105 — 3 file: game_POD_concepts/effects_POD/POD_journeys), "Sự Ôm Ấp" (dòng 2339 — 3 file rải rác), và "Ôm Hôn" (biến thể mới nhất do agent đoạn 2 tự đặt khi dịch `struggle_POD`, dòng 274 gốc, chưa từng có tiền lệ). **Đây là xung đột có TRƯỚC việc #12** — 2 khái niệm loc khác nhau (`pod_Embrace` và `game_concept_Embrace`) đã được dịch khác nhau ngay từ khi tạo (game_POD_concepts dùng "Hiến Máu" bất chấp cùng file có gọi `$pod_Embrace$` trỏ về "Trao truyền" của glossary — mâu thuẫn nội tại ngay trong 1 file). Đã hỏi lại người dùng — **quyết định: dùng "Trao truyền"** (theo nguồn glossary gốc, lan truyền rộng nhất) cho `struggle_POD`. Đã sửa 2 chỗ "Ôm Hôn"→"Trao truyền" (dòng 274 gốc, cả 2 lần xuất hiện trong cùng 1 dòng `pod_assamite_struggle.1006.desc`). **CHƯA sửa `game_POD_concepts_l_english.yml`/`effects_POD_l_english.yml`/`POD_journeys_l_english.yml`** (ngoài phạm vi 6 file việc #12 hiện tại) — còn nợ, cần thống nhất về "Trao truyền" khi có dịp đụng lại các file đó, hoặc hỏi lại người dùng xem có muốn đổi hẳn game_concept_Embrace sang "Trao truyền" để khớp glossary không.

...
**Việc #2 (`religion/`) tiến độ: 21/36 file xong** — thêm `POD_religion_calomenasforsaken`(`917b421`) đã tính ở trên, cộng `POD_religion_bloodgnosticism`(`6126548`), `POD_religion_roadofblood`(`0fe45f0`), `POD_religion_roadofset`(`30c2110`), `POD_religion_roadoflilith`(`8aef336`), `POD_religion_raktasadhus`(`f9a2fb5`), `POD_religion_roadofbones`(`b10442b`), `POD_religion_vampirespecialdoctrines`(`3fa8ac8`). Còn 15 file: `demons`, `docrtineandtenets`(1652 dòng — lớn nhất), `fae`, `gaia`(860 dòng), `mummy`, `ordos`, `roadofbeast`, `roadofheaven`, `roadofhumanity`, `roadofkings`, `roadofmetamorphosis`, `roadofsin`, `roadofzarathustra`, `wraiths`(750 dòng), `wyrm`(243 dòng), `modern`.

(bản ghi cũ, giữ để tham chiếu) — `POD_religion_kueijin` (436/436, `2952a68`), `POD_religion_framework` + `POD_religion_unexamined_horizons` (`50b5f57`), `POD_religion_roadoflaibon` (`dbc28f0`), `POD_religion_ghoul` + `POD_religion_ashirra` + `POD_religion_ignorance` (`7354a25`), `POD_religion_l_english.yml` (439/439, `2ed606c` — file neo thứ hai, khóa cứng 13 tên "Road of X"→"Con Đường X" và 169 dòng `holy_site_*_effect_name`, dùng làm tham chiếu bắt buộc khi dịch các file `roadof*` riêng lẻ còn lại), `POD_religion_calomenasforsaken` (`917b421`), `POD_religion_talmahera` + `POD_religion_naktanchara` + `POD_religion_codeoftremere` + `POD_religion_hunter` (`2f2943f`), `POD_religion_bloodgnosticism` (`6126548`). Còn 21 file khác.

⚠️ **Đã chốt: cụm `"principle of faith"`** (dùng trong `calomenasforsaken`, `bloodgnosticism`, và sẽ gặp lại ở `roadofkings`/`roadofhumanity`/`roadofheaven`) → dịch thống nhất là **"Nguyên Tắc Đức Tin"**. Một agent đã dịch lệch thành "nguyên lý đức tin" ở `bloodgnosticism`, phải sửa lại — kiểm tra kỹ khi tới 3 file `roadof*` còn lại.
